...
export class Drone implements IDrone {
  ...
  getPositonAtTime(t: number): Vector3 {
    ...
    return new Vector3().copy(
      this.positionKeyFrames[this.positionKeyFrames.length - 1].getPos(),
    );
  }
  ...
  removePositionKeyFrame(keyFrame: PositionKeyFrame): void {
    this.positionKeyFrames = this.positionKeyFrames.filter(
      (e) =>
        e.getTime() != keyFrame.getTime() || e.getPos() != keyFrame.getPos(),
    );
  }
  ...
  getColorAtTime(t: number): Color {
    ...
    return this.colorKeyFrames[this.colorKeyFrames.length - 1]
      .getColor()
      .clone();
  }
  ...
  removeColorKeyFrame(keyFrame: ColorKeyFrame): void {
    this.colorKeyFrames = this.colorKeyFrames.filter(
      (e) =>
        e.getTime() != keyFrame.getTime() ||
        e.getColor() != keyFrame.getColor(),
    );
  }
}

function insert<T extends PositionKeyFrame | ColorKeyFrame>(
  keyFrames: T[],
  keyFrame: T,
) {
  ...
  if (
    keyFrames.length !== 0 &&
    keyFrames[keyFrames.length - 1].getTime() === keyFrame.getTime()
  ) {
