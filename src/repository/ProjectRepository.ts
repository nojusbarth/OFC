...
export class ProjectRepository implements IProjectRepository {
  ...
  private dayTime: DayTime = DayTime.NIGHT;
  private endTime: number = 0;

  load(
    input: File | string | null,
    onFinished: (result: Result<boolean>) => void,
  ) {
    if (input == null) {
      this.drones = [];
    ...
    if (input instanceof File) {
      ...
      reader.onloadend = (e: ProgressEvent<FileReader>) => {
        ...
        if (content && !(content instanceof ArrayBuffer)) {
          ...
          onFinished(
            new Result(false, new Error(`Failed to load project: ${e}`)),
          );
        }
      };
  ...
  loadLastProject(): Result<boolean> {
    ...
    if (data == null) {
      return new Result(
        false,
        new Error(`Projekt konnte nicht geladen werden`),
      );
    }
  ...
  private parseJson(content: string): Result<ProjectConfig | null> {
    ...
    try {
      ...
      return new Result(
        null,
        new Error(
          `Projekt kann nicht geladen werden aufgrund der fehlerhaften Syntax`,
        ),
      );
    }
    if (data.version !== FILE_VERSION) {
      return new Result(
        null,
        new Error(
          `Projekt hat die falsche Version: ${data.version} wird nicht unterstützt`,
        ),
      );
    }
  ...
  getNextDroneId(): number {
    ...
    this.drones.forEach((drone) => {
      if (drone.getId() > max) max = drone.getId();
    });
    return max + 1;
  }
  ...
  saveToLocalStorage(): Result<boolean> {
    try {
      ...
      return new Result(
        false,
        new Error(`Projekt kann nicht lokal gespeichert werden`),
      );
    }
  }
}
