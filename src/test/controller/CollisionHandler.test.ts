...
import {
    checkCollisions
} from "../../controller/logic/CollisionHandlerInterface";
import { Drone } from "../../controller/logic/Drone";

it("CollisionHandler static collision", async () => {
    ...
    const collisions = await checkCollisions(drone1, [drone1, drone2], 5);
    expect(collisions.size).toBe(1);
    expect(collisions.keys()).toContain(2);
});

it("CollisionHandler no collision", async () => {
    const drone1 = new Drone(1);
    const drone2 = new Drone(2);
    ...
    const collisions = await checkCollisions(drone1, [drone1, drone2], 4.5);
    expect(collisions.size).toBe(0);
});

it("CollisionHandler moving collision", async () => {
    const drone1 = new Drone(1);
    const drone2 = new Drone(2);
    ...
    const collisions = await checkCollisions(drone1, [drone1, drone2], 1);
    expect(collisions.size).toBe(1);
    expect(collisions.keys()).toContain(2);
